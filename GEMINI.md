...
2. **Tooling:** There are test scripts under `test/*.sh`. Some of them have static ("golden") output in the corresponding `.txt` file. For others, like `apt-prog` the golden output generated on the fly to match the dimensions of the virtual terminal used by Emacs. Use `grep` to look up Emacs built-in functions definitions in `emacs-source/`.
...
3. **Verify:** Work in two modes: either you are implementing a test, or you are making the test to pass. When fixing the code to pass tests, prohibited to modify anything in the `test/` directory. Run `verify.sh` to check the effect of your changes. If there are Elisp errors, analyze and fix them before going any further. After implementing a task, there must be strictly no regressions to other tests, to avoid a vicious loop of endless re-fixing.
