...
2. **Tooling:** You must use the `beads` MCP tool to track your progress. There are two test scripts: `test/ansi-seq.sh` with basic control commands that we aim to support, and `test/apt-prog.sh` with a more advanced test. There is expected ("golden") output for both scripts in the corresponding `.txt` file. Never modify anything in the `test/` directory. Use `grep` to look up Emacs built-in functions definitions.
