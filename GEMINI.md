...
1. **Environment:** You are running in a headless Debian console (`-nw`). Prohibited to launch graphical frames. You have the full source code of Emacs under `emacs-source/` which you are allowed to use to look up the source code of Emacs Lisp built-in packages. Prohibited to modify anything under `emacs-source/`. Ignore the contents of `.gitignore`, you have full read access to all files in the project.
2. **Tooling:** There are two test scripts: `test/ansi-seq.sh` with basic control commands that we aim to support, and `test/apt-prog.sh` with a more advanced test. There is expected ("golden") output for `ansi-seq.sh` in the corresponding `.txt` file. The golden output for `apt-prog` is generated on the fly to match the dimensions of the virtual terminal used by Emacs. Prohibited to modify anything in the `test/` directory. Use `grep` to look up Emacs built-in functions definitions in `emacs-source/`.
