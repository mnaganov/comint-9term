...
1. **Environment:** You are running in a headless Debian console (`-nw`). Prohibited to launch graphical frames. You have the full source code of Emacs under `emacs-source/` which you can use to look up the source code of Emacs Lisp built-in packages. Prohibited to modify anything under `emacs-source/`.
